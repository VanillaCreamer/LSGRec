...
from lightSGCN import LightSignedGCN
from baselines import GCNBase
    ...
    def train(self):
        ...
        for index in range(1):
            ...
            model = LightSignedGCN(data_class.num_u, data_class.num_v, num_layer=args.num_layer, dim=args.dim,
                                   reg=args.reg)
            # model = GCNBase(data_class.num_u, data_class.num_v, num_layer=args.num_layer, dim=args.dim,
            #                        reg=args.reg)
            model.to(device)
            ...
            model.train()
            # self.logger.logging("data: %s" % train)
            # user_pos = train[train['userId'] == 1][train['rating'] > 3.5]['movieId'].values - 1
            # self.logger.logging("user_pos: %s" % user_pos)
            ...
            edge_n = torch.stack((torch.cat((edge_user_n, edge_item_n), 0), torch.cat((edge_item_n, edge_user_n), 0)),
                                 0)
            ...
            for epo in range(1, args.epoch + 1):
                ...
                for u, v, w, negs in ds:
                    ...
                    q += len(u)
                    optimizer.zero_grad()
                    loss = model.loss(u, v, w, negs, data_p, data_n)  # original
                    loss.backward()
                    optimizer.step()
                ...
                if epo % args.freq == 1:
                    ...
                    if args.aggregate == 'siren':
                        ...
                        # emb_p, emb_n, _, _, _, _ = model.aggregate_u_two_embeddings(data_p, data_n)
                        ...
                        # emb_u = emb_u.cpu().detach()
                        # emb_v = emb_v.cpu().detach()
                        # emb_n_u = emb_n_u.cpu().detach()
                        # emb_n_v = emb_n_v.cpu().detach()
                        # np.save('emb_u.npy', emb_u)
                        # np.save('emb_v.npy', emb_v)
                        r_hat = emb_u.mm(emb_v.t()).cpu().detach()
                        r_hat_n = emb_n_u.mm(emb_n_v.t()).cpu().detach()
                        # r_hat = r_hat - r_hat_n  # 1+4+this 0.1170
                        # reco = gen_top_k_new3(data_class, r_hat, r_hat_n)  # filter 3
                        reco = gen_top_k(data_class, r_hat)  # no filter
                        # reco = get_top_k(data_class, r_hat, r_hat_n)  # filter 0
